...
const HomeAdmin = () => {
  ...
  const totalPostApproved = posts?.filter(
    (post) => post?.status === "approved"
  ).length;
  const totalPostPending = posts?.filter(
    (post) => post?.status === "pending"
  ).length;
  const totalPostDestroy = posts?.filter(
    (post) => post?.status === "destroy"
  ).length;
  const totalCategories = categories?.length || 0;
  const totalCategoriesApproved = categories?.filter(
    (category) => category?.status === "approved"
  ).length;
  const totalCategoriesPending = categories?.filter(
    (category) => category?.status === "pending"
  ).length;
  const totalCategoriesDestroy = categories?.filter(
    (category) => category?.status === "destroy"
  ).length;
  const totalFeedbacks = feedback?.length || 0;
  const totalFeedbacksApproved = feedback?.filter(
    (fb) => fb?.status === "approved"
  ).length;
  const totalFeedbacksPending = feedback?.filter(
    (fb) => fb?.status === "pending"
  ).length;
  const totalFeedbacksDestroy = feedback?.filter(
    (fb) => fb?.status === "destroy"
  ).length;
  const totalCustomers = customers?.length || 0;
  const totalCustomersApproved = customers?.filter(
    (cus) => cus?.status === "approved"
  ).length;
  const totalCustomersPending = customers?.filter(
    (cus) => cus?.status === "pending"
  ).length;
  const totalCustomersDestroy = customers?.filter(
    (cus) => cus?.status === "destroy"
  ).length;
  let dayByCustomer = [];
  let dataByCustomer = [];
  let dayByFeedback = [];
