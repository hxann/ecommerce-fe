...
const Breadcrumb = () => {
  ...
  const className =
    " flex items-center last:after:content-[''] after:content-['/'] after:px-3 last:after:px-0";
  ...
  return (
    ...
    >
      <Link to={`/`} className={className}>
        <p className="text-white font-normal">Trang Chủ</p>
      </Link>
      <Link
        to={layout === "detail" || layout === "info" ? `#` : `/${layout}`}
        className={className}
      >
        <p className="text-white font-bold">{nameLayout || ""}</p>
      </Link>
      {page && (
