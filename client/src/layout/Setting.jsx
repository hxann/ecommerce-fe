...
const Setting = () => {
  ...
  return (
    <>
      ...
      <div
        className={`flex-1 absolute text-black bg-white-cream flex top-full w-full justify-center gap-5 
            transition-all
      ...
      >
        {token ? (
          <>
            <Link className={className} to={`/info/${infoAuth?.slug}`}>
              <UserCircleIcon {...icon} />
            ...
            <hr />
            <div
              className={`cursor-pointer text-red-500 ${className}`}
              onClick={handleLogout}
            >
              <ArrowRightOnRectangleIcon {...icon} /> Đăng xuất
            </div>
          </>
        ) : (
          <>
